...
Local keyword retrieval
  ↓
DeepSeek / LangChain answer generation
  ↓
FastAPI API
  ↓
Nginx + Cloudflare
...
如果本地博客里没有相关内容，系统会回答“当前资料不足”。这说明系统遵守了“只基于给定资料回答”的原则，但体验上不够好。因为用户问的是通用技术问题，完全可以通过联网搜索补充。
...
这样出问题时更容易判断是本地检索、Tavily、DeepSeek、Docker 还是 Nginx 的问题。
...
这是个人网站问答系统很重要的一点。如果模型随便编造，就会让读者误以为我写过某篇文章或做过某个项目，反而降低可信度。
...
tools.estevancyber.net    → root /var/www/tools
```
...
这通常不是 Hugo 出错，而是 Nginx 把主站也转发到了 FastAPI。FastAPI 没有 `/` 路由，所以返回了 JSON 404。
